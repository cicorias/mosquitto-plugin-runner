...
then
    ...
    pkeyopts=rsa_keygen_bits:4096
else
    genpkey_algorithm="EC"
...
function generate_root_ca()
{
    ...
    [ $? -eq 0 ] || exit $?
    chmod 664 "${root_ca_dir}/certs/${root_ca_prefix}.cert.pem"
    [ $? -eq 0 ] || exit $?
...
function generate_intermediate_ca()
{
    ...
    [ $? -eq 0 ] || exit $?
    chmod 664 "${intermediate_ca_dir}/certs/${intermediate_ca_prefix}.cert.pem"
    [ $? -eq 0 ] || exit $?
    ...
    [ $? -eq 0 ] || exit $?
    chmod 664 "${intermediate_ca_dir}/certs/${ca_chain_prefix}.cert.pem"
    [ $? -eq 0 ] || exit $?
...
function generate_device_certificate_common()
{
    ...
    openssl genpkey -algorithm "${genpkey_algorithm}" \
            ...
            -pkeyopt ${pkeyopts}
    [ $? -eq 0 ] || exit $?
    chmod 664 "${certificate_dir}/private/${device_prefix}.key.pem"
    [ $? -eq 0 ] || exit $?
    ...
    openssl req -config "${openssl_config_file}" \
        ...
        -subj "$(makeCNsubject "${common_name}")/OU=IoT/O=Contoso/L=Take/ST=NY/C=US" \
    ...
    [ $? -eq 0 ] || exit $?
    chmod 664 "${certificate_dir}/certs/${device_prefix}.cert.pem"
    [ $? -eq 0 ] || exit $?
